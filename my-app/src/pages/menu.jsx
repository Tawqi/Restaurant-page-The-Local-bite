  ...
  useEffect(() => {
    ...
    cleanCategories.forEach((category) => {
      ...
      axios
        ...
        .catch((err) =>
          console.error(`Error fetching dishes for ${category}:`, err)
        );
    });
  }, [cleanCategories]);
    ...
    <>
      ...
      <div className="mx-5 flex flex-col gap-5">
        <div className="sec1 flex flex-col mt-5 gap-5">
          ...
          <div className="cate flex flex-wrap gap-2 items-center">
            <h2 className="">Category:</h2>
            {cleanCategories.map((cat) => (
              <button
                key={cat}
                onClick={() => setSelectedCategory(cat)}
                className={`px-4 py-2 hover:cursor-pointer rounded-xl ${
                ...
                <div key={cat} className="flex flex-col gap-5">
                  ...
                  <div className="flex flex-wrap justify-evenly md:justify-normal gap-4">
                    {filteredFoods.length > 0 ? (
                      filteredFoods.map((food) => <Card food={food} />)
                    ) : (
