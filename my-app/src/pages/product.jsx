      ...
      <div className="textsec flex flex-col gap-4 px-5 md:justify-between md:">
        <div className="text-sec1 flex items-start justify-between gap-2">
          <div className="name_price flex flex-col gap-2">
            <h1 className="text-2xl font-semibold md:text-3xl">{food.name}</h1>
            <h2 className="text-2xl text-(--primary) md:text-3xl">৳{food.price}</h2>
          </div>
          <button className={`fav-btn flex p-2 rounded-xl items-center justify-center md:hidden ${
        ...
        <div className="text-sec2 flex flex-col gap-4">
          ...
          <ul className="list-disc pl-5 flex flex-col gap-1">
            {food.ingredients.map((item, i) => (
              <li className="text-(--text2) font-thin md:text-lg" key={i}>{item}</li>
            ))}
          </ul>
        </div>
